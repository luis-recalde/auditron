...
1. **Read the function body.** Does it perform a sensitive action (create/modify a user, change a role, touch another user's row, move money, issue a refund/payout)? **Also check its `RETURNS` type first** — a function declared `RETURNS trigger` or `RETURNS event_trigger` cannot be invoked directly via `supabase.rpc()` or `/rest/v1/rpc/<name>` no matter how broad its grants are; PostgREST only exposes callable RPC endpoints for functions with a normal return type. A broad grant on a trigger/event-trigger function (commonly `handle_new_user`-style functions wired to `auth.users`) is a hygiene finding at most (**LOW**) — don't flag it at the same severity as a directly-callable function with the same grant pattern. Verified against a real schema while re-running this phase: two functions with grants identical to a genuinely critical one turned out to be trigger functions, not exploitable the same way — reading `RETURNS` before judging severity is what tells them apart.
